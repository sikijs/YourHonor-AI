...
export default function CitationMapView({ user, onError }: { user: User; onError: (err: string) => void }) {
  ...
  } = useLegalTool<CitationMapResponse>(
    (q, docId, signal) => api.legal.citations(q, docId, signal),
    onError,
  );
    ...
    <div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '2rem' }}>
          <div className="spinner-container"><span className="spinner" /><p>Analyzing citations... ({elapsed}s)</p></div>
      ...
      {result && (
        <div className="brief-result">
          ...
          <ActionBar
            ...
            copied={copied}
            onCopy={() => {
              navigator.clipboard.writeText(resultToPlainText(citationMapHtml(result)));
              setCopied(true);
