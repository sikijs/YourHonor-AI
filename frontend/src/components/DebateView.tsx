...
export default function DebateView({ user, onError }: { user: User; onError: (err: string) => void }) {
  ...
  } = useLegalTool<DebateResponse>(
    (q, docId, signal) => api.legal.debate(q, docId, signal),
    onError,
  );
    ...
    <div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '2rem' }}>
          <LoadingStatus message="Retrieving sources and analyzing arguments" elapsed={elapsed} />
      ...
      {result && (
        <div>
          ...
          <ActionBar
            ...
            copied={copied}
            onCopy={() => {
              navigator.clipboard.writeText(resultToPlainText(debateHtml(result)));
              setCopied(true);
